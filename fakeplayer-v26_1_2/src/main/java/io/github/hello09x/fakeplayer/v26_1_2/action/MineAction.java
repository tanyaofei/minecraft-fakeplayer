...
public class MineAction extends TraceAction {
    ...
    public boolean tick() {
        ...
        if (player.gameMode.getGameModeForPlayer().isCreative()) {
            player.gameMode.handleBlockBreakAction(pos, START_DESTROY_BLOCK, side, player.level().getMaxY(), -1);
            current.freeze = 5;
            broken = true;
        } else if (current.pos == null || !current.pos.equals(pos)) {
            if (current.pos != null) {
                player.gameMode.handleBlockBreakAction(current.pos, ABORT_DESTROY_BLOCK, side, player.level().getMaxY(), -1);
            }
            player.gameMode.handleBlockBreakAction(pos, START_DESTROY_BLOCK, side, player.level().getMaxY(), -1);
            ...
            if (current.progress >= 1) {
                player.gameMode.handleBlockBreakAction(pos, STOP_DESTROY_BLOCK, side, player.level().getMaxY(), -1);
                current.pos = null;
                current.freeze = 5;
                broken = true;
            }
